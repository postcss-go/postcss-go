...
const rspackLoader = resolve(packageRoot, 'dist/index.cjs');
...
function createProject(): string {
  const directory = mkdtempSync(join(tmpdir(), 'postcss-go-rspack-loader-'));
  ...
  return new Promise((resolveCompilation, rejectCompilation) => {
    ...
    compiler.run((error, stats) => {
      compiler.close((closeError) => {
        ...
        else if (!stats) rejectCompilation(new Error('Rspack completed without returning stats'));
...
test('processes CSS and forwards warnings, messages, maps, and AST metadata', async () => {
  ...
  const plugin: AcceptedPlugin = {
    postcssPlugin: 'rspack-loader-contract',
    Once(root, { result }) {
      ...
      result.messages.push({
        ...
        content: 'emitted by @postcss-go/rspack-loader',
      });
    },
  ...
  expect(readFileSync(resolve(directory, 'dist/postcss-go-asset.txt'), 'utf8')).toBe(
    'emitted by @postcss-go/rspack-loader',
  );
  ...
  expect([...stats.compilation.buildDependencies].map((file) => realpathSync(file))).toContain(
...
test('reports postcss-go syntax errors through Rspack', async () => {
  const directory = createProject();
...
`,
  ...
  if (!rule || typeof rule === 'string' || !('use' in rule) || !Array.isArray(rule.use)) {
...
test('disables config map output when Rspack source maps are off', async () => {
  const directory = createProject();
...
test('keeps from/to pinned to the Rspack resource path', async () => {
  const directory = createProject();
  const input = resolve(directory, 'input.css');
