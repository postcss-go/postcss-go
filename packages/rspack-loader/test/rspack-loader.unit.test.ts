...
function createProject(): string {
  const directory = mkdtempSync(join(tmpdir(), 'postcss-go-rspack-loader-unit-'));
...
test('accepts async postcssOptions and exposes rspackLoaderContext', async () => {
  const directory = createProject();
  ...
  const result = await runLoader(
    createContext(directory, {
      ...
      async postcssOptions(api) {
        seen.push(api.mode, api.env, api.file, api.rspackLoaderContext, api.options.sourceMap);
        return {
          config: false,
          plugins: [
            {
