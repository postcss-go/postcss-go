...
import type { LoaderContext, LoaderDefinition } from '@rspack/core';

export interface PostcssGoLoaderApi {
  mode: string;
  file: string;
  rspackLoaderContext: LoaderContext<PostcssGoLoaderOptions>;
  env: string;
...
export interface PostcssGoLoaderOptions {
  /** Enable source-map output. Defaults to Rspack's loader source-map setting. */
  sourceMap?: boolean;
...
const loader: LoaderDefinition<PostcssGoLoaderOptions> = function postcssGoLoader(
  content,
  sourceMap,
) {
  this.cacheable?.();
...
async function resolveLoaderOptions(
  ...
  return await configured({
    ...
    rspackLoaderContext: loaderContext,
...
async function resolveProcessConfiguration(
  ...
  if (!Array.isArray(directPlugins)) {
    throw new TypeError(
      '@postcss-go/rspack-loader requires postcssOptions.plugins to be an array of instantiated plugins',
    );
  }
  ...
  const processOptions: ProcessFileOptions = {
    ...
    ...configOptions,
    ...directOptions,
    // Rspack module identity and watch graph use the real resource path.
...
function configureSourceMap(
  ...
  if (!enabled) {
    // Rspack is not consuming maps; do not leave config `map: true` inline
  ...
  // Rspack consumes the map via the loader callback; keep CSS free of
