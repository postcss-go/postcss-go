import { existsSync } from 'node:fs';
import { dirname, join, resolve } from 'node:path';
import { fileURLToPath } from 'node:url';

const packageRoot = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const bundledBinPath = join(
  packageRoot,
  'dist',
  process.platform === 'win32' ? 'postcss-go-node-api.exe' : 'postcss-go-node-api',
);
const monorepoRoot = resolve(packageRoot, '..', '..');
const monorepoBridgeEntry = join(monorepoRoot, 'cmd', 'postcss-go-node-api', 'main.go');

export function getBundledGoBridgeBinPath() {
  return bundledBinPath;
}

export function resolveGoBridgeServiceOptions() {
  const envBin = process.env.POSTCSS_GO_NODE_API_BIN;
  if (envBin) {
    return { binPath: envBin };
  }

  if (existsSync(bundledBinPath)) {
    return { binPath: bundledBinPath };
  }

  if (existsSync(monorepoBridgeEntry)) {
    return { workingDirectory: monorepoRoot };
  }

  throw new Error(
    'Engine Error: postcss-go bridge binary not found. Reinstall @postcss-go/cli or set POSTCSS_GO_NODE_API_BIN.',
  );
}
