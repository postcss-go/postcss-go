...
function benchSynthetic(name, fn, rules) {
  const bytes =
    rules >= 10_000 ? 700_000 : rules >= 1_000 ? 70_000 : rules >= 10 ? 700 : 0
  let options = { bytes: 0 }
  if (rules >= 10_000) {
...
for (const [label, rules] of workloads) {
  ...
  results.push(
    benchSynthetic(`Parse/${label}`, () => {
      postcss.parse(css)
    }, rules),
  )

  results.push(
    benchSynthetic(`ParseStringify/${label}`, () => {
...
for (const fixture of loadRealWorldFixtures()) {
  results.push(
    bench(`ParseReal/${fixture.id}`, () => {
      postcss.parse(fixture.css)
    }, { bytes: fixture.bytes }),
  )

  results.push(
    bench(`ParseStringifyReal/${fixture.id}`, () => {
