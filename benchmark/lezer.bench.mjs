/**
 * Benchmark the Lezer CSS parser (https://github.com/lezer-parser/css) using
 * the same parse workloads as benchmark/bench_test.go.
 *
...
for (const [label, rules] of workloads) {
  ...
  results.push(benchSynthetic(`Parse/${label}`, () => parser.parse(css), rules));
}

for (const fixture of loadRealWorldFixtures()) {
  results.push(
    bench(`ParseReal/${fixture.id}`, () => parser.parse(fixture.css), {
      bytes: fixture.bytes,
    }),
  );
}
